...
def ctseg_decode(heat, wh, seg_feat, seg ,reg, K=100):
    batch, cat, height, width = heat.size()
    assert batch==1
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    ...
    wh = _tranpose_and_gather_feat(wh, inds)
    wh = wh.view(batch, K, 2)
    weight = _tranpose_and_gather_feat(seg, inds)
    ...
    ## for 1x1
    # weight = weight.squeeze(0).unsqueeze(-1).unsqueeze(-1)
    # pred_seg = F.conv2d(seg_feat, weight)
...
def ctseg2_decode(heat, wh,reg, seg, K=100):
    batch, cat, height, width = heat.size()
    assert batch==1
    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    ...
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    reg = _tranpose_and_gather_feat(reg, inds)
    window_id = (clses.long()*100 + torch.arange(0,100).view(1,100).cuda())
    ind  = window_id.unsqueeze(2).expand(window_id.size(0), window_id.size(1), 2)
    reg = reg.view(batch, K,3,2).permute(0, 2, 1, 3).contiguous().view(batch, K*3,2).gather(1, ind)
    xs = xs.view(batch, K, 1) + reg[:, :, 0:1]
    ys = ys.view(batch, K, 1) + reg[:, :, 1:2]
    wh = _tranpose_and_gather_feat(wh, inds)
    wh = wh.view(batch, K,3,2).permute(0, 2, 1, 3).contiguous().view(batch, K*3,2).gather(1, ind)
    seg = _tranpose_and_gather_feat(seg, inds).sigmoid()

    pred_small, pred_mid, pred_lagre = torch.split(seg, [20 * 20, 30 * 30, 40 * 40], dim=-1)
    pred_small = pred_small.view(batch,K,20,20).cpu().numpy()
    pred_mid = pred_mid.view(batch, K, 30,30).cpu().numpy()
    pred_lagre = pred_lagre.view(batch, K, 40,40).cpu().numpy()
    import  numpy as np
    import cv2
    mask_small=pred_small[0,0]
    mask_mid = pred_mid[0,0]
    mask_lagre = pred_lagre[0,0]
    mask_small = (cv2.resize(mask_small,(32,32)) > 0.5).astype(np.uint8)
    mask_mid = (cv2.resize(mask_mid,(96,96)) > 0.5).astype(np.uint8)
    mask_lagre = (cv2.resize(mask_lagre,(512,512)) > 0.5).astype(np.uint8)

    cv2.imshow('small',mask_small*255)
    cv2.imshow('mid',mask_mid*255)
    cv2.imshow('lagre',mask_lagre*255)
    cv2.waitKey(0)
